...
public class ExpandableHeader extends FrameLayout {
    ...
    @Nullable
    ExpandableNestedScrollView expansionLayout;
    @Nullable
    ...
    public ExpandableHeader(@NonNull Context context) {
        super(context);
        init(context, null);
    }

    public ExpandableHeader(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public ExpandableHeader(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(@NonNull Context context, @Nullable AttributeSet attrs) {
        if (attrs != null) {
            final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ExpandableHeader);
            if (a != null) {
                setHeaderRotationExpanded(a.getInt(R.styleable.ExpandableHeader_expansion_headerIndicatorRotationExpanded, headerRotationExpanded));
                setHeaderRotationCollapsed(a.getInt(R.styleable.ExpandableHeader_expansion_headerIndicatorRotationCollapsed, headerRotationCollapsed));
                setHeaderIndicatorId(a.getResourceId(R.styleable.ExpandableHeader_expansion_headerIndicator, headerIndicatorId));
                setExpansionLayoutId(a.getResourceId(R.styleable.ExpandableHeader_expansion_layout, expansionLayoutId));
                setToggleOnClick(a.getBoolean(R.styleable.ExpandableHeader_expansion_toggleOnClick, toggleOnClick));
                a.recycle();
            }
        }
    }
    ...
    public void setExpansionLayout(@Nullable ExpandableNestedScrollView expansionLayout) {
        ...
        if (expansionLayoutId != 0) {
            ...
            if (parent instanceof ViewGroup) {
                ...
                if (view instanceof ExpandableNestedScrollView) {
                    setExpansionLayout(((ExpandableNestedScrollView) view));
                }
            }
        }
    }
    ...
    private void setup() {
        if (expansionLayout != null && !expansionLayoutInitialised) {
            expansionLayout.addIndicatorListener(new ExpandableNestedScrollView.IndicatorListener() {
                @Override
    ...
    public void addListener(ExpandableNestedScrollView.Listener listener) {
        if (expansionLayout != null) {
            expansionLayout.addListener(listener);
        }
    }

    public void removeListener(ExpandableNestedScrollView.Listener listener) {
        if (expansionLayout != null) {
