...
public class LoadMoreAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private void registerAdapter(RecyclerView.Adapter adapter) {
        ...
        mLoadMore = new LoadMore(mOnEnabledListener);
    }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == TYPE_FOOTER) {
            ...
            View view = LoadMoreHelper.inflate(parent, R.layout.load_more_base_footer);
            ...
            View view = LoadMoreHelper.inflate(parent, R.layout.load_more_base_no_more);
            ...
            if (view == null) {
                view = LoadMoreHelper.inflate(parent, R.layout.load_more_base_load_failed);
            }
            return new LoadFailedHolder(view, mLoadMore, mOnLoadMoreListener);
        }
    ...
    public int getItemCount() {
        ...
        return isLoadMoreEnable() ? count + 1 : mShowNoMoreEnabled ?
    ...
    public int getItemViewType(int position) {
        ...
        if (position == mAdapter.getItemCount() && (isLoadMoreEnable() || mShouldRemove)) {
            return TYPE_FOOTER;
        } else if (position == mAdapter.getItemCount() && mShowNoMoreEnabled && !isLoadMoreEnable()) {
            return TYPE_NO_MORE;
        }
    ...
    public static class LoadFailedHolder extends RecyclerView.ViewHolder {
        ...
        public LoadFailedHolder(View itemView, final LoadMore enabled, final OnLoadMoreListener listener) {
            super(itemView);
            LoadMoreHelper.setItemViewFullSpan(itemView);
    ...
    private RecyclerView.OnScrollListener mOnScrollListener = new RecyclerView.OnScrollListener() {
        ...
        public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
            ...
            if (!isLoadMoreEnable() || mIsLoading) {
                return;
            }
    ...
    public interface OnLoadMoreListener {
        void onLoadMore(LoadMore enabled);
    }

    public void setLoadMoreEnable(boolean enabled) {
        mLoadMore.setEnable(enabled);
    }

    public boolean isLoadMoreEnable() {
        return mLoadMore.isEnable() && mAdapter.getItemCount() >= 0;
    }
    ...
    public void setLoadFailed(boolean isLoadFailed) {
        mLoadMore.setLoadFailed(isLoadFailed);
    }

    /**
     * 控制加载更多的开关, 作为 {@link OnLoadMoreListener#onLoadMore(LoadMore enabled) 的参数}
     */
    public static class LoadMore {
        ...
        public LoadMore(OnEnabledListener listener) {
            mListener = listener;
        }
        ...
        public void setEnable(boolean enable) {
            final boolean canNotify = mLoadMoreEnable;
    ...
    private RecyclerView.AdapterDataObserver mObserver = new RecyclerView.AdapterDataObserver() {
        ...
        public void onItemRangeRemoved(int positionStart, int itemCount) {
            ...
            if (mLoadMore.isEnable() && mAdapter.getItemCount() == 0) {
        ...
        public void onItemRangeMoved(int fromPosition, int toPosition, int itemCount) {
            if (mShouldRemove && (fromPosition == mAdapter.getItemCount() || toPosition == mAdapter.getItemCount())) {
                throw new IllegalArgumentException("can not move last position after setEnable(false)");
            }
