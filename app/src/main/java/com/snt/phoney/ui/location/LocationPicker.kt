...
class LocationPicker : BaseFragment() {
    ...
    private fun search(keyword: String = "", latLng: LatLng? = null, noBound: Boolean = false) {
        if (latLng != null) {
            ...
            mapProxy.reverseGeocode(location).observe(this, Observer {
            ...
            mapProxy.searchPoi(keyword).observe(this, Observer { result ->
                address = null
                fillAdapter(address = address, pois = result)
            })
        } else {
            viewModel.getMyLocation { location ->
                ...
                mapProxy.searchPoi(keyword, Position(location)).observe(this, Observer { result ->
                    fillAdapter(address = address, pois = result)
                })
            }
        }
