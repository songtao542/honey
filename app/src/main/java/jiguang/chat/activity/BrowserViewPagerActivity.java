...
import jiguang.chat.utils.AttachmentStore;
...
public class BrowserViewPagerActivity extends BaseActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        RelativeLayout titleBarRl = findViewById(R.id.title_bar_rl);
        RelativeLayout checkBoxRl = findViewById(R.id.check_box_rl);
    ...
    PagerAdapter pagerAdapter = new PagerAdapter() {
        ...
        public View instantiateItem(ViewGroup container, int position) {
            ...
            if (path != null) {
                File file = new File(path);
                if (file.exists()) {
                    Bitmap bitmap = BitmapLoader.getBitmapFromFile(path, screenWidth, screenHeight);
                    if (bitmap != null) {
...
//                                break;
            ...
            mDialog = DialogCreator.createSavePictureDialog(this, listener);
            mDialog.show();
            mDialog.getWindow().setLayout((int) (0.8 * screenWidth), WindowManager.LayoutParams.WRAP_CONTENT);
            return false;
        });
    }
    ...
    public void savePicture(String path, Dialog dialog) {
        ...
        if (AttachmentStore.copy(path, dstPath) != -1) {
            try {
                ...
                dialog.dismiss();
                Toast.makeText(this, getString(R.string.picture_save_fail), Toast.LENGTH_LONG).show();
            }
        } else {
            dialog.dismiss();
            Toast.makeText(this, getString(R.string.picture_save_fail), Toast.LENGTH_LONG).show();
    ...
    private void setLoadBtnText(ImageContent ic) {
        ...
        String loadText = getString(R.string.load_origin_image) + "(" + ddf1.format(size) + "M" + ")";
    ...
    private void checkPictureSelected(final int currentItem) {
        mPictureSelectedCb.setOnCheckedChangeListener((compoundButton, isChecked) -> {
            if (mSelectMap.size() + 1 <= 9) {
                ...
                Toast.makeText(this, getString(R.string.picture_num_limit_toast), Toast.LENGTH_SHORT).show();
                mPictureSelectedCb.setChecked(mSelectMap.get(currentItem));
            } else {
                mSelectMap.delete(currentItem);
            }
    ...
    private void showTotalSize() {
        if (mSelectMap.size() > 0) {
            ...
            String totalText = getString(R.string.origin_picture)
                    + String.format(getString(R.string.combine_title), totalSize);
            mTotalSizeTv.setText(totalText);
        } else {
            mTotalSizeTv.setText(getString(R.string.origin_picture));
        }
    }
    ...
    private void showSelectedNum() {
        if (mSelectMap.size() > 0) {
            String sendText = getString(R.string.jmui_send) + "(" + mSelectMap.size() + "/" + "9)";
            mSendBtn.setText(sendText);
        } else {
            mSendBtn.setText(getString(R.string.jmui_send));
        }
    }
    ...
    private void initCurrentItem() {
        ...
        try {
            ...
            if (ic.getBooleanExtra("originalPicture") != null && ic.getBooleanExtra("originalPicture")) {
                mLoadBtn.setVisibility(View.GONE);
                setLoadBtnText(ic);
                photoView.setImageBitmap(BitmapLoader.getBitmapFromFile(path, screenWidth, screenHeight));
            } else {
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                ...
                case R.id.pick_picture_send_btn:
                    mProgressDialog = new ProgressDialog(BrowserViewPagerActivity.this);
                    mProgressDialog.setMessage(getString(R.string.sending_hint));
    ...
    private void downloadImage() {
        ...
        if (imgContent.getLocalPath() == null) {
            ...
            if (!mMsg.isContentDownloadProgressCallbackExists()) {
                ...
                mProgressDialog.setMessage(getString(R.string.downloading_hint));
                mDownloading = true;
                mProgressDialog.show();
                // 显示下载进度条
                mMsg.setOnContentDownloadProgressCallback(new ProgressUpdateCallback() {
