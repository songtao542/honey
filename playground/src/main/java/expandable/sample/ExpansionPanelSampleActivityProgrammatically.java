...
import expandable.widget.ExpandableCollection;
import expandable.widget.ExpandableHeader;
import expandable.widget.ExpandableNestedScrollView;

import static expandable.sample.Utils.dpToPx;
...
public class ExpansionPanelSampleActivityProgrammatically extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final ExpandableNestedScrollView ex1 = addDynamicLayout();
        final ExpandableNestedScrollView ex2 = addDynamicLayout();

        //example of how to add a listener
        ex1.addListener(new ExpandableNestedScrollView.Listener() {
            @Override
            public void onExpansionChanged(ExpandableNestedScrollView expansionLayout, boolean expanded) {

            }
        });

        final ExpandableCollection expansionLayoutCollection = new ExpandableCollection();
        expansionLayoutCollection.add(ex1).add(ex2);
        expansionLayoutCollection.openOnlyOne(true);
    }

    public ExpandableNestedScrollView addDynamicLayout() {

        final ExpandableHeader expansionHeader = createExpansionHeader();
        dynamicLayoutContainer.addView(expansionHeader, ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);

        final ExpandableNestedScrollView expansionLayout = createExpansionLayout();
    ...
    @NonNull
    private ExpandableNestedScrollView createExpansionLayout() {
        final ExpandableNestedScrollView expansionLayout = new ExpandableNestedScrollView(this);
    ...
    @NonNull
    private ExpandableHeader createExpansionHeader() {
        final ExpandableHeader expansionHeader = new ExpandableHeader(this);
